...
const NewsAndEventsCreate = ({
  openAddNewsAndEvents,
  setopenAddNewsAndEvents,
  onNewsAndEventsCreated,
}) => {
  ...
  const onClick = async () => {
    try {
      const response = await axios.post('/newsandevents', items);
      if (response.status === 201) {
        toast.success('News And Events Added', {
          autoClose: 1500,
          onClose: () => {
            onNewsAndEventsCreated(); // Trigger the refresh
  ...
  return (
    <>
      ...
      <div className="p-4">
        <div className="">
          <label>NewsAndEvents Name</label>
          <Input
            type="text"
            placeHolder="Name"
          ...
          />
          <label>NewsAndEvents Heading</label>
          <Input
            type="text"
            placeHolder="Heading"
            onChange={e => onChange(e, 'heading')}
          />
          <label>NewsAndEvents Description</label>
          <TextArea
            type="text"
          ...
          <div className="right-inputs">
            <label>NewsAndEvents Slider Image</label>
...
export default NewsAndEventsCreate;
